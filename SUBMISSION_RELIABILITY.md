...
**Description:**
```
174 tests across 9 files: test_health.py, test_unit.py, test_unit_modules.py, test_coverage_gaps.py, test_urls.py, test_users.py, test_events.py, test_integration_routes.py, test_performance.py. All pass on every PR with 91% coverage. Screenshot shows the bot-posted test results on PR #12. Full docs: https://retr0-os.github.io/MLH_PE_URL_Shortener/Reliability/RELIABILITY_ENGINEERING#unit-tests
```
...
**Description:**
```
unit-tests.yml GitHub Actions workflow runs on every PR to dev or main. Spins up real Postgres + Redis service containers, runs the full pytest suite with --cov-fail-under=70, and posts coverage report + test summary as a bot comment directly on the PR. Merge is blocked if any test fails. Full docs: https://retr0-os.github.io/MLH_PE_URL_Shortener/Reliability/RELIABILITY_ENGINEERING#ci-runs-tests-on-every-pr
```
...
**Description:**
```
91% coverage — CI enforces --cov-fail-under=70. If coverage drops below 70%, pytest exits non-zero and the PR cannot merge. Screenshot shows the github-actions bot posting 91% coverage report on PR #12. Dedicated coverage-gap tests cover error handler branches (503, 405, 500) that happy-path tests miss. Full docs: https://retr0-os.github.io/MLH_PE_URL_Shortener/Reliability/RELIABILITY_ENGINEERING#coverage--50-actual-91
```
...
**Description:**
```
91% coverage — 21 points above the Gold threshold. CI gate enforces --cov-fail-under=70 on every PR. 9 test files collectively cover 91% of application code lines. Dedicated coverage-gap tests (test_coverage_gaps.py) explicitly test the 503, 405, and 500 error branches. Full docs: https://retr0-os.github.io/MLH_PE_URL_Shortener/Reliability/RELIABILITY_ENGINEERING#coverage--70-actual-91
```
...
**Description:**
```
Full failure modes table documenting 9 scenarios: app container crash (Docker restart in 5s, second replica absorbs traffic), OOM kill (restart + HighMemoryUsage alert), PostgreSQL down (503 + pool reconnect), Redis down (circuit breaker, zero 5xx, cache refills on recovery), bad client input (400/409/410, no state change), duplicate insert (IntegrityError → 409, no corruption), N+1 regression (caught by test_performance.py in CI), replica death under load (Nginx routes around it), broken PR code (merge blocked), deploy health check failure (old containers keep running, team notified via GitHub Actions). Full docs: https://retr0-os.github.io/MLH_PE_URL_Shortener/Reliability/RELIABILITY_ENGINEERING#failure-modes-documentation
```

---

## ALERTING EVIDENCE (Observability / Incident Response cross-track)

### Alert channel delivery (email + Discord)
...
**Screenshots:**
- `alert-email-resend-dashboard-delivered.png` — Resend dashboard showing all alert emails delivered (HighReplicaCount, HighLatency warnings)
- `alert-email-gmail-high-replica-count.webp` — Gmail inbox showing HighReplicaCount warning email with full severity/summary/description
- `alert-email-gmail-high-latency.webp` — Gmail inbox showing HighLatency warning email (p95 > 500ms, current value 0.66s)
- `alert-discord-channel-warnings.png` — Discord #alerts channel with ALERT King bot posting HighReplicaCount and HighLatency warnings

**Description:**
```
Alertmanager routes alerts to two independent channels: email (via Resend SMTP) and Discord (via webhook). Critical alerts fire instantly (group_wait: 0s), warnings batch at 10s intervals. Screenshots show real alerts firing for HighReplicaCount (autoscaler scaled to 4 replicas under load) and HighLatency (p95 > 500ms). Email uses rich HTML templates. Discord uses severity-color-coded messages. Both channels confirmed delivering under 90s from alert trigger.
```
